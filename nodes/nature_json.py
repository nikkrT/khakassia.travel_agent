...
from promts.nodes_promt import CHOICE_CLASS_PROMPT_NATURE,CHOICE_PROMPT


# --- 1. Конфигурация ---
JSON_FILE_PATH = "data/nature.json" # Путь к вашему файлу с категориями


router_prompt_template = ChatPromptTemplate.from_messages([
    ("system", CHOICE_CLASS_PROMPT_NATURE),
...
def nature_extract(state: AgentState):
    ...
    try:
        ...
        chosen_category = str(chosen_category)
        print(f"-> Выбрана категория: '{chosen_category}'")
    except Exception as e:
        print(f"Ошибка при выборе категории: {e}. Использую 'остальное'.")
        state[
            "result"] = "К сожалению, у меня возникли технические проблемы с доступом к базе природы Хакасии. Попробуйте позже."
        return state
    ...
    try:
        ...
        state[
            "result"] = "К сожалению, у меня возникли технические проблемы с доступом к базе данных по природе Хакасии. Попробуйте позже."
        return state
    ...
    with open('usage.txt', 'a', encoding='utf-8') as f:
        f.write(f'nature json extraction usage: {usage}\n')
